const menu = [
  ...
  function categoryButtons() {
    const allButtons = document.createElement("ul");
    allButtons.className = "list-inline";
    ...
    buttonArray.forEach((buttonName) => {
      ...
      btn.className = "btn btn-outline-warning d-inline ";
      btn.id = buttonName;
      btn.innerText = buttonName;
      btn.addEventListener("click", () => showFoods(buttonName));
  
      li.appendChild(btn);
      allButtons.append(li);
    });
  
    filterButtons.appendChild(allButtons);
  }
  ...
  function showFoods(categoryName) {
    ...
    menu.forEach((food) => {
      if (categoryName == "ALL") {
        allFoods += foodBlock(food);
      } else if (food.category == categoryName) {
        allFoods += foodBlock(food);
      }
    });
  ...
  document.addEventListener("DOMContentLoaded", () => showFoods("ALL"));